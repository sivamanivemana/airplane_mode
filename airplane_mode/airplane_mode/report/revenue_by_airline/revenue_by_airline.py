...
def execute(filters=None):
    ...
    tickets = frappe.db.get_all("Airplane Ticket", fields=[
                                'name', 'total_amount'])
    records = []
    revenue_by_airline = {}

    for ticket in tickets:
        records.append(frappe._dict(
            {"name": ticket['name'][:-33], "total_amount": ticket['total_amount']}))

    for record in records:
        if (record.name) in revenue_by_airline:
            revenue_by_airline[record.name] = revenue_by_airline[record.name] + \
                record.total_amount
